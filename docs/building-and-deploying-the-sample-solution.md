...
- ScenarioSimulator.Tests and Core.Tests (in the UnitTests folder). These projects contain unit tests to verify that the simulator is running correctly *What do these tests actually show? TBD* ...
...
5. Configure HDInsight (optional).
...
- Using the Azure web portal, manually create the following items:
...
- On the Stream Analytics page, verify that a Stream Analytics job has been created using the name that you specified when you ran the provisioning script (the name that you entered in response to the *StreamAnalyticsJobName* prompt).
...
- Return to the job page, and then in the menu bar click Outputs.
...
### Configuring HDInsight (Optional)
...
- In the New dialog box, select Hadoop. Enter a cluster name and admin password of your choice, specify the storage account that you used when you ran the provisioning script, and then click Create HDInsight Cluster.

> **Note:** It can take several minutes to create the cluster.
...
The UnitTests project ... (*What do these tests demonstrate? - TBD*)

To run the unit tests, on the Visual Studio menu bar click Test, click Run, and then click All Tests.

All tests should succeed without errors.
...
- Select your Stream Analytics job, and on the command bar click Start.
...
- Allow the simulator to run for a couple of minutes and then press q.

- Press Enter to return to the menu.

- Repeat the previous three steps for options 2 to 5. As before, the simulator will generate and echo events and no exceptions should occur.

- On the menu, select option 6 to quit the simulator.

### Validating the Stream Analytics output
...
### Validating the Blob Data by Using a Hive Query (Optional)

Perform the following steps to process the data in blob storage by using a Hive query:
...
-  At the *clusterName* prompt, enter the name of the Hadoop cluster that you created in the section Configuring HDInsight.
...
- *NEED MORE INFORMATION HERE - WHERE DO THE QUERY RESULTS GO?*
